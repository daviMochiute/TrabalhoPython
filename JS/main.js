...
const insertDataToCardsJson = (data, container) => {
    const containerForDiv = document.getElementById('cardDestaque');
    clearContainer(containerForDiv);
    dados = data.results
    dados.forEach(element => {
        containerForDiv.appendChild(createCard(element));
...
const randomCard = (container) => {
    ...
    const url = `../dados.json`;
    fetch(url).then(response => response.json()).then(data => insertDataToCardsJson(data));
}
